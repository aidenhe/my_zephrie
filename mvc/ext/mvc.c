...
zend_class_entry *mvc_applications_ce;
zend_class_entry *mvc_autoload_ce;
zend_class_entry *mvc_bigpipe_ce;
zend_class_entry *mvc_controller_ce;

ZEND_DECLARE_MODULE_GLOBALS(mvc)
...
static PHP_MINIT_FUNCTION(mvc)
{
	...
	ZEPHIR_INIT(Mvc_Applications);
	ZEPHIR_INIT(Mvc_Autoload);
	ZEPHIR_INIT(Mvc_Bigpipe);
	ZEPHIR_INIT(Mvc_Controller);
...
#ifndef ZEPHIR_RELEASE
static PHP_MSHUTDOWN_FUNCTION(mvc)
{
 ...
 */
static void php_zephir_init_globals(zend_mvc_globals *zephir_globals TSRMLS_DC)
{
...
static PHP_RINIT_FUNCTION(mvc)
{

	zend_mvc_globals *zephir_globals_ptr = ZEPHIR_VGLOBAL;

	php_zephir_init_globals(zephir_globals_ptr TSRMLS_CC);
	//zephir_init_interned_strings(TSRMLS_C);

	zephir_initialize_memory(zephir_globals_ptr TSRMLS_CC);
	zend_printf("Rinit_log_helonghua");
	return SUCCESS;
}

static PHP_RSHUTDOWN_FUNCTION(mvc)
{
...
static PHP_MINFO_FUNCTION(mvc)
{
	php_info_print_box_start(0);
	php_printf("%s", PHP_MVC_DESCRIPTION);
	php_info_print_box_end();

	php_info_print_table_start();
	php_info_print_table_header(2, PHP_MVC_NAME, "enabled");
	php_info_print_table_row(2, "Author", PHP_MVC_AUTHOR);
	php_info_print_table_row(2, "Version", PHP_MVC_VERSION);
	php_info_print_table_row(2, "Powered by Zephir", "Version " PHP_MVC_ZEPVERSION);
	php_info_print_table_end();


}

static PHP_GINIT_FUNCTION(mvc)
{
	php_zephir_init_globals(mvc_globals TSRMLS_CC);
}

static PHP_GSHUTDOWN_FUNCTION(mvc)
{

}

zend_module_entry mvc_module_entry = {
